...
```
    ...
    - Create a `.env` file inside `server`. Follow the instruction given :
    ```env
        # Environment
...
-   `dashboard` contains the dashboard feature (Web Platform)
...
-   `blog_app` folder contains a flutter based android application.
    -   Open the project in Android SDK
    -   Run it on emulator or any android device on debugger mode.
...
-   Initial Story upload and Bangla Generate (POST) `/story/initial`
...
-   Final story update (PUT) `/story/final/:id`
...
-   Contribution create (POST) `/improvement/create`
...
-   Get all contribution (GET) `/improvement/getAll`
...
-   Update improvment by id (PUT) `/improvement/:postid`
